...
for k = 1: numel(fileList)
    ...
    if k == 1 && ~(k == numel(fileList))
        ...
        % Identify if nearest index of timestep is equal to first timestep of next file
    ...
    % Sort importTimeIdx 'ascend'. LF 09/28: Only last index necessary? "Sample points must be sorted in ascending order"
    % fileList(k).importTimeIdx                                   = sort(fileList(k).importTimeIdx,'ascend');
